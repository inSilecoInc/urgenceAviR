...
load_Iles_Nunavik <- function() {
  ...
  Iles_Nunavik <- Iles_Nunavik |>
    dplyr::mutate(
      code_id = ifelse(
        !is.na(taxo$Species_ID[match(Iles_Nunavik$Nom_scientifique, taxo$Scientific_Name)]),
        taxo$Species_ID[match(Iles_Nunavik$Nom_scientifique, taxo$Scientific_Name)],
        ifelse(
